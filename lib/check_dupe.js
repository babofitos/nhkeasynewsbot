...
function CheckDupe() {
  ...
  this.on('data', function(data, pos) {
    //pos 0 from nhk json. pos 1 from reddit json
    logger.log('debug', 'checkdupe %j for %d', data, pos);
    this.kids[pos] = data;
    logger.debug('this.index', this.index);
    //check if both nhk json and reddit json emitted
    if (++this.index === this.max) {
      logger.debug('Filtering article ids');
      //filter out all ids found in reddit selftext from nhk json
      //should be left with non dupe articles
      ...
      if (complement.length < 1) {
        ...
        logger.log('debug', 'Non dupe article ids %j', complement);
